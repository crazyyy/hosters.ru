	...
	with( this ){
		...
		this.preview_container.find( 'a.otw-shortcode-control-edit' ).click( function(){
			...
			if( ( row_id > -1 ) && ( column_id > -1 ) && ( shortcode_id > -1 ) ){
				...
				jQuery.post( 'admin-ajax.php?action=otw_post_template_shortcode_editor_dialog&shortcode=' + shortcode_type, { shortcode_object: post_data, otw_item_id: item_id }, function(b){
					...
					otw_shortcode_editor.preview = otw_post_template_shortcode_component.shortcodes[shortcode_type].object.preview;
					...
					tb_show( get_label( 'Modify' ) + ' OTW ' + otw_post_template_shortcode_component.shortcodes[shortcode_type].title, "#TB_inline?width="+f+"&height="+b+"&inlineId=otw-dialog" );
				});
			}
		} );
...
otw_post_template_grid_manager_row = function(){

	this.columns = new Array();
};

otw_post_template_grid_manager_column = function( rows, from_rows, mobile_rows, mobile_from_rows ){
...
otw_post_template_grid_manager_shortcode = function( settings ){
...
otw_post_template_grid_manager_shortcode.prototype.html_code = function( object_name, row_id, column_id, shortcode_id, parent, total_shortcodes ){
