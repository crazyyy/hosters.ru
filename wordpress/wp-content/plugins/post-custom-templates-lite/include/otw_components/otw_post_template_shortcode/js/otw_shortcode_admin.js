		...
		for( var shortcode in this.shortcodes ){
			...
			if( this.shortcodes[ shortcode ].enabled && !this.shortcodes[ shortcode ].parent ){
				...
				if( this.shortcodes[ shortcode ].children ){
					...
					if( ( typeof( this.shortcodes[ shortcode ].options ) != 'undefined' ) && ( this.shortcodes[ shortcode ].options == false ) ){
						links = links + '<li><a class="otw-shortcode-dropdown-action-' + shortcode + '" data-options="no">' + this.shortcodes[ shortcode ].title + '</a></li>';
					}else{
...
otw_shortcode_object.prototype.init_dropdown_actions = function(){
	...
	with( this ){
		...
		jQuery( 'body' ).click( function( event ){
			...
			if( jQuery( event.target ).parents( '.mce-btn' ).attr( 'data-otwkey' ) == otw_post_template_shortcode_component.tinymce_button_key ){
				close_it = false
			};
			if( jQuery( event.target ).parents( '.mce_' + otw_post_template_shortcode_component.tinymce_button_key ).size() ){
				close_it = false;
			};
				...
				if( matches = jQuery( this ).attr( 'class' ).match( /^otw\-shortcode\-dropdown\-action\-([a-zA-Z0-9_\-]+)$/ ) ){
					...
					switch( matches[1] ){
						...
						case 'close':
							...
							break;
						default:
								if( jQuery( this ).attr( 'data-options' ) == 'no' ){
									
									jQuery.post( wp_url + '.php?action=otw_post_template_shortcode_get_code&shortcode=' + matches[1],{'shortcode_type': matches[1]},function( response ){
										
										var response_code = jQuery.parseJSON( response );
										
										var code = {};
										code.shortcode_code = '';
										code.shortcode_type = matches[1];
									...
									jQuery.get( wp_url + '.php?action=otw_post_template_shortcode_editor_dialog&shortcode=' + matches[1],function(b){
...
otw_shortcode_editor_object.prototype.live_reload = function(){
	...
	with( this ){
		...
		jQuery.post( wp_url + '.php?action=otw_post_template_shortcode_live_reload&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
...
otw_shortcode_editor_object.prototype.live_preview = function(){
	...
	with( this ){
		...
		jQuery.post( wp_url + '.php?action=otw_post_template_shortcode_live_preview&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
...
otw_shortcode_editor_object.prototype.get_code = function(){
	...
	with( this ){
		...
		jQuery.post( wp_url + '.php?action=otw_post_template_shortcode_get_code&shortcode=' + this.shortcode_type , this.code, function( response ){
...
otw_ovelay_shortcode_component = null;
