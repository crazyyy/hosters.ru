<?php
class OTW_Post_Template_Shortcode extends OTW_Component{
	...
	public function include_shortcodes(){
		...
		foreach( $this->shortcodes as $shortcode_key => $shortcode_settings ){
			...
			if( !is_array( $shortcode_settings['children'] ) || !count( $shortcode_settings['children'] ) ){
				...
				if( !class_exists( 'OTW_Post_Template_ShortCode_'.$shortcode_key ) ){
				...
				$class_name = 'OTW_Post_Template_ShortCode_'.$otw_shortcode_key;
				...
				foreach( $this->shortcodes[ $shortcode_key ]['object']->external_libs as $lib_array ){
					...
					if( method_exists( $this, 'add_lib' ) && $this->combine_libs ){
						...
						if( preg_match( "/include\/otw_components/", $lib_array['path'], $matches ) ){
							$deps = array();
							$this->add_lib( $lib_array['type'], 'otw_post_template_'.$lib_array['name'], $lib_array['path'], $lib_array['int'], $lib_array['order'], $deps );
						}else{
							$this->add_external_lib( $lib_array['type'], 'otw_post_template_'.$lib_array['name'], $lib_array['path'], $lib_array['int'], $lib_array['order'], $lib_array['deps'] );
						}
					}else{
						$this->add_external_lib( $lib_array['type'], 'otw_post_template_'.$lib_array['name'], $lib_array['path'], $lib_array['int'], $lib_array['order'], $lib_array['deps'] );
					}
	...
	public function init(){
		...
		if( is_admin() ){
			...
			add_action( 'wp_ajax_otw_post_template_shortcode_editor_dialog', array( &$this, 'build_shortcode_editor_dialog' ) );
			add_action( 'wp_ajax_otw_post_template_shortcode_get_code', array( &$this, 'get_code' ) );
			add_action( 'wp_ajax_otw_post_template_shortcode_live_preview', array( &$this, 'live_preview' ) );
			add_action( 'wp_ajax_otw_post_template_shortcode_live_reload', array( &$this, 'live_reload' ) );
			add_action( 'wp_ajax_otw_post_template_shortcode_preview_shortcodes', array( &$this, 'preview_shortcodes' ) );
			add_action( 'wp_ajax_otw_post_template_shortcode_preview_front_shortcodes', array( &$this, 'preview_front_shortcodes' ) );
	...
	public function load_admin_js(){
		...
		$js .= "otw_post_template_shortcode_component = new otw_shortcode_object();";
		$js .= "otw_post_template_shortcode_component.shortcodes = ".$this->format_js_shortcodes().";";
		$js .= "otw_post_template_shortcode_component.labels = ".json_encode( $this->labels ).";";
		$js .= "otw_post_template_shortcode_component.tinymce_button_key='".$this->tinymce_button_key."';";
		$js .= "otw_post_template_shortcode_component.wp_url='admin-ajax'";
		$js .= "</script>";
	...
	public function load_front_js(){
		...
		$js .= "otw_post_template_shortcode_component.labels = ".json_encode( $this->labels ).";";
		$js .= "otw_post_template_shortcode_component.tinymce_button_key='".$this->tinymce_button_key."';";
		$js .= "otw_post_template_shortcode_component.wp_url='".get_site_url()."/wp-admin/admin-ajax'";
		$js .= "</script>";
	...
	public function build_shortcode_editor_dialog(){
		...
		if( $shortcode && isset( $shortcode['has_options'] ) && !$shortcode['has_options'] ){
			echo 'da';
		}
		elseif( $shortcode ){
	...
	public function include_admin_scripts(){
		...
		wp_enqueue_script('otw_post_template_shortcode_admin', $this->component_url.'js/otw_shortcode_admin.js' , array( 'jquery' ), $this->js_version );
		wp_enqueue_style( 'otw_post_template_shortcode_admin', $this->component_url.'css/otw_shortcode_admin.css', array( ), $this->css_version );
	}
	...
	public function format_js_shortcodes(){
		...
		foreach( $this->shortcodes as $shortcode_key => $shortcode_data ){
			...
			if( isset( $shortcode_data['options'] ) ){
				$shortcodes[ $shortcode_key ]['options'] = $shortcode_data['options'];
			}
