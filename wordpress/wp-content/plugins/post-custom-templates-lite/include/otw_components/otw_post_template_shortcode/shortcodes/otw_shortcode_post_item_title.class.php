<?php
class OTW_Post_Template_Shortcode_Post_Item_Title extends OTW_Post_Template_Shortcodes{
	...
	public function __construct(){
		...
		$this->shortcode_name = 'otw_shortcode_post_item_title';
	}
	...
	public function build_shortcode_code( $attributes ){
		...
		if( !$this->has_error ){
			...
			$code = '[otw_shortcode_post_item_title]';
		}
		...
		if( is_admin() ){
			...
			if( $post_item_id ){
				...
				if( is_array( $otw_post_items_data ) && isset( $otw_post_items_data[ $post_item_id ] ) && isset( $otw_post_items_data[ $post_item_id ]['data'] ) ){
					...
					if( $otw_post_items_data[ $post_item_id ]['data']['settings']['otw_pct_item_title'] == 'yes' ){
						$html .= $otw_post_items_data[ $post_item_id ]['dispatcher']->getTitle( $otw_post_items_data[ $post_item_id ]['data']['post'] );
					}
				}
			}
		}
