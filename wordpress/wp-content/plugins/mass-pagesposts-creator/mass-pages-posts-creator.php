<?php
	...
	if ((isset($_GET['page']) && ((!empty($_GET['page']) || ('mass-pages-posts-creator' === $_GET['page'])))) || (isset($_GET['tab']) && ((!empty($_GET['tab']) || ('other_plugins' === $_GET['tab']))))) {
	...
	if ((isset($_GET['page']) && !empty($_GET['page']) && ('mass-pages-posts-creator' === $_GET['page'])) || (isset($_GET['page']) && !empty($_GET['page']) && ('mass-pages-post-creator' === $_GET['page']) && isset($_GET['tab']) && !empty($_GET['tab']) && ('about' === $_GET['tab']))) {
	...
	add_action( 'plugin_row_meta', 'mppc_plugin_row_meta',10,2 );
	function mppc_plugin_row_meta( $links, $file ) {

		if ( strpos( $file, 'mass-pages-posts-creator.php' ) !== false ) {
			$new_links = array(
	...
	function welcome_screen_content_mass_page_creator()
	{
		...
		?>
        <div class="wrap about-wrap mppc">
            <h1><?php printf(__('Welcome to Mass Pages/Posts Creator', 'mass-pagesposts-creator')); ?></h1>
            <div class="about-text woocommerce-about-text">
                ...
                <img class="version_logo_img"
                     src="<?php echo esc_url(plugin_dir_url(__FILE__) . 'images/mass-pagesposts-creator.png'); ?>">
            </div>
			<?php
				$setting_tabs_wc = apply_filters('mass_page_post_creator_setting_tab', array("about" => "Overview"));
            ...
            <h2 id="woo-extra-cost-tab-wrapper" class="nav-tab-wrapper">
				<?php
					foreach ($setting_tabs_wc as $name => $label)
						echo '<a href="' . esc_url(home_url('wp-admin/index.php?page=mass-pages-post-creator&tab=' . esc_attr($name))) . '" class="nav-tab ' . ($current_tab_wc == $name ? 'nav-tab-active' : '') . '">' . esc_attr($label) . '</a>';
				?>
            </h2>
			<?php
            ...
            <div class="changelog about-integrations">
                <div class="wc-feature feature-section col three-col">
                    <div>
                        ...
                        <p class="mass_page_post_creator_overview">
                            <strong><?php _e('Key Features:', 'mass-pages-posts-creator'); ?></strong></p>
            ...
            <form id="createForm" method="post" class="">
                ...
                <table class="form-table">
                    ...
                    <tr class="pages_list_tr">
                        <th><?php _e('List of Pages/Posts', 'mass-pages-posts-creator'); ?></br><?php _e('(Comma Seperated)', 'mass-pages-posts-creator'); ?>
                            <b>(*)</b></th>
                        <td>
                    ...
                    <tr class="pages_content_tr">
                        ...
                        <td>
                            ...
                            readable content of a page when looking at its layout.', 'mass-pages-posts-creator'); ?></p>
                        </td>
                    </tr>
                    <tr class="excerpt_content_tr">
                        ...
                        <td><textarea class="code" id="excerpt_content" cols="60" rows="5"
                            ...
                            <p class="description"><?php _e('eg. It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.', 'mass-pages-posts-creator'); ?></p>
                        </td>
                    </tr>
                    <tr class="page_prefix_tr">
											...
											if (isset($subpages) || !empty($subpages)) {
												foreach ($subpages as $subpage) {
                                                    ...
                                                    <option style="margin-left: 10px;"
                                                            value="<?php echo esc_attr($subpage->ID); ?>"><?php echo ' -- ' . wp_kses_post($subpage->post_title); ?></option>
													<?php
													...
													if (isset($childsubpages) || !empty($childsubpages)) {
														foreach ($childsubpages as $childsubpage) {
															?>
                                                            <option style="margin-left: 30px;"
                                                                    value="<?php echo esc_attr($childsubpage->ID); ?>"><?php echo ' -- ' . wp_kses_post($childsubpage->post_title); ?></option>
															<?php
														}
													}
												}
                        ...
                        <td>
                            <select id="page_status">
                                ...
                                <option value="publish"
                                        disabled><?php _e('Publish - In Pro', 'mass-pages-posts-creator'); ?></option>
                    ...
                    <tr class="comment_status_tr">
                        ...
                        <td>
                            <select id="comment_status">
                                ...
                                <option value="open"
                                        disabled><?php _e('Open - In Pro', 'mass-pages-posts-creator'); ?></option>
                                <option value="closed"
                                        disabled><?php _e('Closed - In Pro', 'mass-pages-posts-creator'); ?></option>
                            </select>
                        </td>
                    </tr>
                    <tr class="authors_tr">
                ...
                <p class="submit">
                    <input type="button" id="btn_submit" class="button button-primary" name="btn_submit"
                           value="<?php esc_html_e('Generate now', 'mass-pages-posts-creator'); ?>"/>
                    <img style="display: none;"
                         src="<?php echo plugin_dir_url(__FILE__) . '/images/ajax-loader.gif'; ?>" name="loader_gif"
                         id="loader_gif">
                </p>
            </form>
            <div id="message"></div>
